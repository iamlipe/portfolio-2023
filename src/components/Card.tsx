  ...
  return (
    <a href="/" className="w-1/2 h-[74%]" {...rest}>
      ...
      <div className="flex flex-col gap-4 relative w-full h-full p-1 tracking-wider leading-tight">
        <span className="text-base lg:text-lg xl:text-xl font-light tracking-wider leading-tight underline">
          ⌗00
          {index + 1}
        </span>
        <div className="flex relative flex-col h-full justify-between">
          <p className="line-clamp-4 font-extralight text-xs lg:text-sm xl:text-base tracking-wider leading-tight">{description}</p>
          <h4 className="absolute uppercase top-[74%] -left-24 text-2xl md:text-3xl lg:text-4xl -rotate-90 w-60 text-start whitespace-nowrap">{title}</h4>
        </div>
      </div>
    </a>
  );
